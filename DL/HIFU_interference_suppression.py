...
def load_cond(path):
  RF_cond = sio.loadmat(path)['fftData']
...
def denoiseV2(DP, diffuser, path_label, path_input, device, n):  # fft as cond
  ssim_f = SSIM()
  label = load_rf(path_label, True)
  input = load_rf(path_input, False)
  cond = load_cond(path_input)
  input = input.to(device=device, dtype=torch.float32)
  cond = cond.to(device=device, dtype=torch.float32)
  label = label.to(dtype=torch.float32)
  with torch.no_grad():
    noise_pre = diffuser.sample(DP, cond, n)
    output = (input + noise_pre).cpu()
    if n != 1:
      label = label.repeat(n, 1, 1, 1)
    idx_ssim = ssim_f(output, label)
  ...
  with torch.no_grad():
    for epoch in range(epoches):
      ...
      for batch in tqdm(dataloader, total=num_val_batches, desc='Validation round', unit='batch', leave=False):
        ...
        if use_ddim:
          noise_pre = diffuser.sample_ddim(DP, input, 1, ddim_step=ts_ddim, eta=eta_ddim,disable=True)
        else:
...
def denoise_dataset_mg(net, diffuser, save_path, dataloader, epoches, device, val_sampler,use_ddim=False,ts_ddim=20,eta_ddim=0.5):
  ...
  file_path = path_save + 'output1.txt'
  local_rank = int(os.environ["LOCAL_RANK"])
...
def predict_dataset_normal(net, path_save, dataloader, epoches, device):
  num_val_batches = len(dataloader)
  ssim_val = 0
  ssim_f = SSIM()
...
def get_args():
  ...
  parser.add_argument('--model', '-m', default='total_epoch_2000', metavar='FILE',
  ...
  parser.add_argument('--rf', '-r', default='0717_76',
  ...
  parser.add_argument('--eta_ddim', '-eta', default=0.0, type=int, help='DDIM eta')
...
if __name__ == '__main__':
  ...
  if hparams['predict']['mgpu']:
    ...
    logging.basicConfig(level=logging.INFO, format='%(asctime)s %(levelname)s %(message)s', handlers=[
      logging.FileHandler('../store_pre/output1.txt'),  # 将日志保存到文件中
  ...
  if hparams['dataset']['name'] == '0717_fft2Dcond':
    input_root = hparams['dataset']['dir_val'] + '/inputwithfft2/'
  else:
  ...
  if hparams['predict']['ddpmMode']:
    if mode == 3:
      ...
      if hparams['dataset']['name'] == '0717_fft2Dcond':
        denoiseV2(DP, diffuser, dir_label, dir_input, device, args.num)
      elif hparams['predict']['MA_predict']:
    ...
    if mode == 2:
      ...
      loader_args = dict(batch_size=args.batch_size, num_workers=os.cpu_count(), pin_memory=True)
      val_loader = DataLoader(val_set, shuffle=False, drop_last=False, **loader_args)
      predict_dataset_normal(DP, path_save, val_loader, args.epoches, device)
    elif mode == 1:
