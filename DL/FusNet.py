...
class Conv2df(nn.Module):
  def __init__(self, in_channels, out_channels):
    super().__init__()
    self.seq = nn.Sequential(
      nn.Conv2d(in_channels, out_channels, kernel_size=3, padding=1, bias=False, stride=1),
...
class SeqDown(nn.Module):  # 前3层使用
  def __init__(self, in_channels):
    super().__init__()
    self.seq = nn.Sequential(
      nn.Conv2d(in_channels, in_channels, kernel_size=4, padding=1, bias=False, stride=1),
      nn.BatchNorm2d(in_channels),
      nn.LeakyReLU(inplace=True),
      nn.Conv2d(in_channels, in_channels, kernel_size=4, padding=3, bias=False, stride=2),
      nn.BatchNorm2d(in_channels),
      nn.LeakyReLU(inplace=True)
    )
...
class Down(nn.Module):
  def __init__(self, in_channels, out_channels):
    ...
    self.down = nn.Sequential(
      SeqDown(in_channels),
      Conv2df(in_channels, out_channels)
    )
...
class Up(nn.Module):
  def __init__(self, in_channels, out_channels):
    ...
    self.conv = Conv2df(in_channels, out_channels)
